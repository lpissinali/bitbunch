...
function scrollIntoView() {
    ...
    const currencyInitialOffset = ensureItemVisibleOffset(currentTrade.currency, state.columns.currencies);
...
function showInstantTradeSellProgress() {
    const timeline = new anime.timeline();
...
function showInstantTrade() {
    ...
    return timeline.finished.then(showInstantTradeSellProgress);
}

function closeInstantTrade() {
...
function closeLongTrade() {
    ...
    timeline.add(scrollColumn(state.columns.currencies,
        offscreenItemOffset(currentTrade.currency, state.columns.currencies) + SELECTED_EXCHANGE_WIDTH * 0.5,
        CLOSE_DURATION,
        updateCurrencies), 0);
